...
//           socialAccounts
//         });
//         navigate('/influencer/InfluencerDashboard');
//       } else {
//         console.log({
//           email,
//           name,
...
//         navigate('/company/InfluencerDashboard');
//       }
//     } catch (err) {
//       alert('Error: ' + err.message);
//     }
...
const Signup = () => {
  ...
  const { isOpen, onOpen, onClose } = useDisclosure();
  const navigate = useNavigate();
  const { login } = useAuth(); // Use the useAuth hook instead of useContext(AuthContext)
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const userData = {
        ...
        role,
        ...(role === 'influencer'
          ? { categories, socialAccounts }
          : { contactInfo, industry, companySize, website, verificationDocs: verificationDocs ? verificationDocs.name : null }),
      };

      console.log('Signup Data:', userData);

      // Update AuthContext with user data (mock login)
      ...
      >
        ...
        >
          ...
          <VStack spacing={4} align="stretch">
            ...
            <FormControl isRequired isInvalid={error && password !== passwordConfirm}>
              <Input
                placeholder="Confirm Password"
                type="password"
                value={passwordConfirm}
            ...
            {role === 'influencer' && (
              <>
                <FormControl isInvalid={error && !socialAccounts.instagram && !socialAccounts.tiktok && !socialAccounts.youtube}>
                  <FormLabel>Connect Social Accounts</FormLabel>
                  <VStack spacing={3}>
                    <SocialPlatformConnect 
                      platform="Instagram" 
                  ...
                  {socialAccounts.tiktok && (
                    <Text mt={2} fontSize="sm">
                      Connected: {socialAccounts.tiktok.username} ({socialAccounts.tiktok.followers.toLocaleString()} followers)
                    </Text>
                  )}
                  {socialAccounts.youtube && (
                    <Text mt={2} fontSize="sm">
                      Connected: {socialAccounts.youtube.username} ({socialAccounts.youtube.followers.toLocaleString()} followers)
                    </Text>
                  )}
                </FormControl>

                <FormControl isInvalid={error && categories.length === 0}>
                  <FormLabel>Interest Categories (max 3)</FormLabel>
                  <SimpleGrid columns={2} spacing={2}>
                    {influencerCategories.map(category => (
                      <Button
            ...
            {role === 'company' && (
              <>
                <FormControl isRequired isInvalid={error && !contactInfo}>
                  <FormLabel>Contact Information (Phone)</FormLabel>
                  <Input
                    placeholder="Phone Number"
                    type="tel"
                ...
                <FormControl isRequired isInvalid={error && !industry}>
                  <FormLabel>Industry</FormLabel>
                  <Select
                    placeholder="Select industry"
                    value={industry}
                ...
                <FormControl isRequired isInvalid={error && !companySize}>
                  <FormLabel>Company Size</FormLabel>
                  <Select
                    placeholder="Select company size"
                    value={companySize}
                ...
                <FormControl isRequired isInvalid={error && !website}>
                  <FormLabel>Website</FormLabel>
                  <Input
                    placeholder="https://yourcompany.com"
                    type="url"
            ...
            <FormControl isInvalid={error && !termsAccepted}>
              <Checkbox
                isChecked={termsAccepted}
                onChange={(e) => setTermsAccepted(e.target.checked)}
                colorScheme="red"
